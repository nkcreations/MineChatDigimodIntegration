...
public class EnvironmentScanner {
    ...
    public static String findNearestStructure(Entity center, int radius) {
        ...
        for (TagKey<Structure> tag : tagsToSearch) {
            var holderSet = serverLevel.registryAccess().registryOrThrow(Registries.STRUCTURE).getOrCreateTag(tag);
            var result = serverLevel.getChunkSource().getGenerator().findNearestMapStructure(serverLevel, holderSet, centerPos, radius, false);
            if (result != null) {
